...
<title>CV SHENUODA</title>
<link href="https://fonts.googleapis.com/css2?family=Cairo:wght@300;400;600;700&display=swap" rel="stylesheet">
<style>
body{
  margin:0;
  font-family:'Cairo',sans-serif;
  background: linear-gradient(135deg, #071029, #1a1f3d, #3b3f5c);
...
@keyframes floatBubble{
  ...
  50% { transform: translate(-2px,3px) scale(0.98); }
...
.contact-buttons a:hover{transform:scale(1.05);}
...
<p style="margin-top:10px; font-weight:600;">وكان الرب مع يوسف فكان رجلًا ناجحًا.</p>
</section>
...
for(let i=0;i<50;i++){
  ...
  const size=Math.random()*6+4;b.style.width=size+'px';b.style.height=size+'px';
  b.style.left=Math.random()*100+'%';
  b.style.animationDuration=(5+Math.random()*5)+'s';
  b.style.background=`hsl(${Math.random()*360},80%,70%)`;
  b.style.animationDelay=Math.random()*5+'s';
  bubbleBg.appendChild(b);
  setInterval(()=>{b.style.background=`hsl(${Math.random()*360},80%,70%)`;},2000+Math.random()*3000);
}

// تحميل الصفحة + شريط مهارات + عداد
window.addEventListener('load',()=>{
  document.body.classList.add('loaded');
  document.querySelectorAll('.counter').forEach(c=>{
    const target=parseInt(c.dataset.target); let count=0;
    const interval=setInterval(()=>{if(count<target){count++;c.textContent=count+'%';}else clearInterval(interval);},15);
  });
});
